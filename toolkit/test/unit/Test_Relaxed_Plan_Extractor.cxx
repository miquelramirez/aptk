#include "Test_Relaxed_Plan_Extractor.hxx"
...
CPPUNIT_TEST_SUITE_REGISTRATION( Test_Relaxed_Plan_Extractor );

void Test_Relaxed_Plan_Extractor::setUp()
{
...
void Test_Relaxed_Plan_Extractor::tearDown()
{
	delete the_problem;
}

void Test_Relaxed_Plan_Extractor::test_correctness_max_heuristic()
{
	aig_tk::Relaxed_Plan_Extractor<aig_tk::Max_Heuristic> rp_ex;
	rp_ex.initialize( *the_problem );
	CPPUNIT_ASSERT_EQUAL( (unsigned)4, rp_ex.eval( the_problem->init(), the_problem->goal() ) );
}

void Test_Relaxed_Plan_Extractor::test_correctness_additive_heuristic()
{
	aig_tk::Relaxed_Plan_Extractor<aig_tk::Additive_Heuristic> rp_ex;
	rp_ex.initialize( *the_problem );
	CPPUNIT_ASSERT_EQUAL( (unsigned)4, rp_ex.eval( the_problem->init(), the_problem->goal() ) );
}
